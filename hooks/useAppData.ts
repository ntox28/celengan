...
import { supabase, Customer, Employee, Bahan, Expense, Order, OrderItem, Payment, User, Bank, Printer, Asset, Debt, NotaSetting, Supplier, StockMovement, Finishing } from '../lib/supabaseClient';
...
export const useAppData = (user: User | undefined) => {
    ...
    const createRecord = async <T extends {id: number}>(table: string, data: any, setData: React.Dispatch<React.SetStateAction<T[]>>, successMessage: string) => {
    ...
    const updateRecord = async <T extends {id: number}>(table: string, id: number, data: any, setData: React.Dispatch<React.SetStateAction<T[]>>, successMessage: string) => {
        const { data: updatedRecord, error } = await supabase.from(table).update(data).eq('id', id).select().single();
        if (error) { addToast(`Gagal: ${error.message}`, 'error'); throw error; }
        setData(prev => prev.map(item => (item.id === id ? updatedRecord as T : item)));
        addToast(successMessage, 'success');
    };
    ...
    // --- Customers ---
    const addCustomer = async (data: any) => createRecord('customers', data, setCustomers, 'Pelanggan berhasil ditambahkan.');
    const updateCustomer = async (id: number, data: any) => updateRecord('customers', id, data, setCustomers, 'Pelanggan berhasil diperbarui.');
    const deleteCustomer = async (id: number) => deleteRecord('customers', id, setCustomers, 'Pelanggan berhasil dihapus.');

    // --- Employees ---
    const addEmployee = async (data: any) => createRecord('employees', data, setEmployees, 'User berhasil ditambahkan.');
    const updateEmployee = async (id: number, data: any) => updateRecord('employees', id, data, setEmployees, 'User berhasil diperbarui.');
    const deleteEmployee = async (id: number) => deleteRecord('employees', id, setEmployees, 'User berhasil dihapus.');

    // --- Bahan ---
    const addBahan = async (data: any) => createRecord('bahan', data, setBahanList, 'Bahan berhasil ditambahkan.');
    const updateBahan = async (id: number, data: any) => updateRecord('bahan', id, data, setBahanList, 'Bahan berhasil diperbarui.');
    const deleteBahan = async (id: number) => deleteRecord('bahan', id, setBahanList, 'Bahan berhasil dihapus.');

    // --- Banks ---
    const addBank = async (data: any) => createRecord('banks', data, setBanks, 'Sumber dana berhasil ditambahkan.');
    const updateBank = async (id: number, data: any) => updateRecord('banks', id, data, setBanks, 'Sumber dana berhasil diperbarui.');
    const deleteBank = async (id: number) => deleteRecord('banks', id, setBanks, 'Sumber dana berhasil dihapus.');

    // --- Printers ---
    const addPrinter = async (data: any) => createRecord('printers', data, setPrinters, 'Printer berhasil ditambahkan.');
    const updatePrinter = async (id: number, data: any) => updateRecord('printers', id, data, setPrinters, 'Printer berhasil diperbarui.');
    const deletePrinter = async (id: number) => deleteRecord('printers', id, setPrinters, 'Printer berhasil dihapus.');

    // --- Assets ---
    const addAsset = async (data: any) => createRecord('assets', data, setAssets, 'Aset berhasil ditambahkan.');
    const updateAsset = async (id: number, data: any) => updateRecord('assets', id, data, setAssets, 'Aset berhasil diperbarui.');
    const deleteAsset = async (id: number) => deleteRecord('assets', id, setAssets, 'Aset berhasil dihapus.');

    // --- Debts ---
    const addDebt = async (data: any) => createRecord('debts', data, setDebts, 'Data hutang berhasil ditambahkan.');
    const updateDebt = async (id: number, data: any) => updateRecord('debts', id, data, setDebts, 'Data hutang berhasil diperbarui.');
    const deleteDebt = async (id: number) => deleteRecord('debts', id, setDebts, 'Data hutang berhasil dihapus.');

    // --- Suppliers ---
    const addSupplier = async (data: any) => createRecord('suppliers', data, setSuppliers, 'Suplier berhasil ditambahkan.');
    const updateSupplier = async (id: number, data: any) => updateRecord('suppliers', id, data, setSuppliers, 'Suplier berhasil diperbarui.');
    const deleteSupplier = async (id: number) => deleteRecord('suppliers', id, setSuppliers, 'Suplier berhasil dihapus.');

    // --- Finishings ---
    const addFinishing = async (data: any) => createRecord('finishings', data, setFinishings, 'Finishing berhasil ditambahkan.');
    const updateFinishing = async (id: number, data: any) => updateRecord('finishings', id, data, setFinishings, 'Finishing berhasil diperbarui.');
    ...
    const updateExpense = async (id: number, data: any) => updateRecord('expenses', id, data, setExpenses, 'Pengeluaran berhasil diperbarui.');
    ...
    const addOrder = async (orderData: any) => {
        ...
        orderPayload.no_nota = newNotaNumber;
        ...
        const itemsPayload = order_items.map((item: any) => ({...item, order_id: newOrder.id}));
    ...
    const updateOrder = async (id: number, orderData: any) => {
        ...
        // 3. Insert new items
        const itemsPayload = order_items.map((item: any) => ({ ...item, order_id: id, id: undefined, created_at: undefined, local_id: undefined }));
    ...
    const addPaymentToOrder = async (orderId: number, paymentData: any) => {
